...
local strip_ansi = function(text)
...
local build_subl_target = function(selection)
	local clean = strip_ansi(selection)
	if clean == "" then
		return
	end
	...
	local target = path
	if line then
		target = string.format("%s:%s", target, line)
		if col then
			target = string.format("%s:%s", target, col)
		end
	end
...
local launch_sublime = function(selection, cwd)
	local target = build_subl_target(selection)
	if not target then
		return
	end

	local cmd = Command("subl")
	if not cmd then
		return fail("`subl` command not found in PATH")
	end
	...
	if not child then
		fail("Failed to launch Sublime Text: %s", err or "unknown error")
	end
end
	...
	local cmd_tbl = {
		rg = {
			...
			prev = "--preview='bat --color=always "
				.. opts_tbl.bat
				.. " --highlight-line={2} {1}' --preview-window=~3,+{2}+3/2,up,80%",
			prompt = "--prompt='rg> '",
